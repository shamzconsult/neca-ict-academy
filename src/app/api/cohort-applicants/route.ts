...
export async function GET(request: Request) {
  try {
    ...
    .populate<{ course: { _id: Types.ObjectId; title: string } }>('course', 'title')
    .populate<{ cohort: { _id: Types.ObjectId; name: string } }>('cohort', 'name')
    .sort({ createdAt: -1 })
    .lean<IEnrollment[]>();
    ...
    const enrichedApplicants: IApplicantResponse[] = applicants.map(applicant => {
      ...
      return {
        ...
        course: (latestEnrollment?.course as { title: string })?.title || 'No course',
        cohort: (latestEnrollment?.cohort as { name: string })?.name || 'No cohort',
