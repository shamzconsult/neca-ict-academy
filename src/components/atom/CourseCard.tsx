...
export const CourseCard = ({
  ...
  const handleDelete = async (slug: string, event: React.MouseEvent) => {
    ...
    try {
      ...
      setCoursesData(prevCourse => prevCourse.filter(course => course.slug !== slug));
      ...
      {coursesData.length > 0 ? (
        <div
          className={`grid justify-center gap-8 ${
            isCoursesPath || admin ? 'grid-cols-1 md:grid-cols-2 lg:space-x-16' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3'
          }`}>
          {coursesData.map(course => (
            <CardWrapper
              ...
              className={`bg-white border border-[#C4C4C480] mx-auto ${
                ...
                isCoursesPath || admin ? 'w-full md:w-[350px] lg:w-[500px] hover:cursor-pointer ' : ''
              }`}>
              <div className={`  relative  p3 ${isCoursesPath || admin ? 'w-full  h-[250px] lg:h-[270px]' : 'w-full h-[250px]  lg:h-[189px] '}`}>
                <Image
              ...
              <h3 className='text-lg font-bold text-[#27156F] mt-4'>{course.title}</h3>
              <div>
                {(isCoursesPath || admin) && <p className='font-bold mt-2 text-sm'>About The Course</p>}
                <p className={`mt-1 ${isCoursesPath ? 'text-[12px]' : ' text-sm'} break-words overflow-hidden text-ellipsis`}>{course.description}</p>
                <div>
                  {(isCoursesPath || admin) && (
                      ...
                      <div className='grid grid-cols-3 gap-3 text-xs'>
                        ...
                        <div className='flex flex-col justify-center items-center px-4 py-1.5 border border-[#7272721A] rounded-lg'>
                          ...
                          <div className='flex  gap-2 items-center '>
                            ...
                            <span className='text-nowrap'>{course.skillLevel}</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className='mt-5'>
                {(isCoursesPath || admin) && (
                  <p>
                    <span className='font-bold'>{course.rating}</span>⭐ <span>({course.review})</span>
                  </p>
                )}
              </div>
