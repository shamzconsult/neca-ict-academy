...
export const ApplicantTr = ({ enrollment }: Props) => {
  ...
  const { _id, firstName, lastName, email, course, level, state, status, createdAt } = enrollment;
    ...
    <tr
      ...
      <td className='p-4'>
        <button
          ...
          <span className='text-sm text-nowrap'>{email}</span>
        </button>
      </td>
      <td className='p-4 text-nowrap'>{state}</td>
      <td className='p-4 text-nowrap'>{new Date(createdAt).toDateString()}</td>
      <td className='p-4 text-nowrap'>{String(course)}</td>
