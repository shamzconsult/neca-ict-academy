...
export const CohortPreview = ({ slug }: { slug: string }) => {
  const [searchTerm, setSearchTerm] = useState("");

  const cohortData = cohorts.find((cohort) => {
    return cohort.slug == slug;
  });
  ...
  const filteredData = cohortData.applicantDetails?.filter((item) =>
