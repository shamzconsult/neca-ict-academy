            ...
            <tbody>
              {cohortsData.map((cohort: CohortType) => (
                <tr
                  ...
                  <td className={`p-4 ${cohort.active ? ' text-green-600' : ' text-[#e02b20]'}`}>{cohort.active ? 'Active' : 'Inactive'}</td>
                  <td className='p-4'>
                    <CgMoreVertical />
                    {hoveredRow === cohort._id && (
                      <div className='absolute right-10 mt-2 font-semibold bg-white text-white border  border-[#C4C4C4] rounded-md shadow-lg w-32 px-2'>
                        <div className='py-1 flex flex-col gap-2'>
                          ...
                          <button
                            onClick={event => handleDelete(cohort.slug, event)}
